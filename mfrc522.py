...
class MFRC522:
    ...
    def halt_a(self):
        ...
        return self._tocard(0x0C, buf)[0]
    ...
    def read(self, addr, into = None):
        ...
        buf[1]=addr
        self._assign_crc(buf, 2)
        (stat, recv, bits) = self._tocard(0x0C, buf, into=into)
        if stat != self.OK or bits < 128:
            return None
    ...
    def write(self, addr, data):
        ...
        if not (stat == self.OK) or not (bits == 4) or len(recv) < 1 or not ((recv[0] & 0x0F) == 0x0A):
            stat = self.ERR
        else:
