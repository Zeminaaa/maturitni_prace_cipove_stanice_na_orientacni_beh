...
# --- 1. FÁZE: SYNCHRONIZACE ---
# Program se na tomto řádku ZASTAVÍ.
# Dokud deska nechytí signál od Mastera, dál nepůjde.
# Jakmile chytí signál -> nastaví RTC -> zahraje melodii -> a pustí kód dál.
...
def write(TARGET_BLOCK,KEY,uid):
    #raw_uid = [0, 0, 0, 0] 
    if uid==UID:
        try:
            while True:
                    ...
                    if stat == rdr.OK:
                        ...
                        if rdr.select_tag(raw_uid) == rdr.OK:
                            ...
                            if rdr.auth(rdr.AUTHENT1A, TARGET_BLOCK, KEY, raw_uid) == rdr.OK:
                                ...
                                print(f"Zapisuji čas: {current_timestamp}")
                                data=uprav_data(current_timestamp,data_z_blocku)
...
# ... (inside the main execution block)
# Získat aktuální čas (který byl nastaven synchronizací)
while True:
    reset_sensor(rst)
    spi = SPI(2, baudrate=2500000, polarity=0, phase=0, sck=Pin(sck), mosi=Pin(mosi), miso=Pin(miso))
